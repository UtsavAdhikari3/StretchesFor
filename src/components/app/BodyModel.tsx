...
export default function BodyModel({ selectedId, onSelect, locale = 'en' }: Props) {
  ...
  const modelChildren = bodyRegions.map((region) => createElement(
    'button',
    {
      ...
      'aria-label': `${t(locale, 'Select')} ${t(locale, region.label)}`,
    ...
    createElement('span', { className: 'hotspot-label' }, t(locale, region.label)),
  ));
  ...
  const viewer = status === 'loading' || status === 'ready'
    ? createElement('model-viewer', {
        ...
        alt: t(locale, 'Interactive neutral human body model for selecting a pain region'),
  ...
  return (
    <div>
        ...
        {(status === 'checking' || status === 'loading') && (
          <div className="pointer-events-none absolute inset-0 grid place-items-center bg-canvas/72 p-8 text-center" aria-live="polite">
            <div>
              ...
              <p className="mt-4 text-sm font-semibold">{t(locale, 'Loading body model…')}</p>
              <p className="mt-1 text-xs text-subtle">{t(locale, 'The region list remains available while it loads.')}</p>
            </div>
          </div>
        )}
        {(status === 'missing' || status === 'error') && (
          <div className="absolute inset-0 grid place-items-center p-8 text-center" role="status">
            <div className="max-w-sm">
              ...
              <h3 className="mt-5 text-lg font-semibold">{t(locale, 'Human model unavailable')}</h3>
              <p className="mt-2 text-sm leading-6 text-muted">{t(locale, 'The 3D model could not be loaded. Select any region from the accessible text list and try reloading the page.')}</p>
            </div>
          </div>
        )}
        <div className="pointer-events-none absolute inset-x-0 top-0 flex items-start justify-between gap-3 p-3 sm:p-4">
          <span className="rounded-lg border border-line bg-surface/90 px-3 py-2 text-xs font-semibold shadow-sm backdrop-blur-md">{t(locale, 'Tap a point to select')} <span aria-hidden="true">·</span> {t(locale, 'Drag to rotate')}</span>
          {selected && <span className="max-w-[55%] rounded-lg border border-brand/40 bg-brand/12 px-3 py-2 text-right text-xs font-semibold text-brand shadow-sm backdrop-blur-md">{t(locale, 'Selected')} · {t(locale, selected.label)}</span>}
        </div>
        <div className="absolute bottom-3 right-3 grid gap-1 rounded-xl border border-line bg-surface/92 p-1 shadow-lg backdrop-blur-md sm:bottom-4 sm:right-4" role="group" aria-label={t(locale, 'Zoom and reset 3D body')} aria-controls={viewerId}>
          <button type="button" aria-label={t(locale, 'Zoom in')} onClick={() => zoomCamera('in')} disabled={status !== 'ready'} className="grid size-11 place-items-center rounded-lg text-xl font-medium text-muted transition-[color,background-color,transform] hover:bg-surface-raised hover:text-ink active:scale-95 disabled:cursor-wait disabled:opacity-40 motion-reduce:active:scale-100">+</button>
          <button type="button" aria-label={t(locale, 'Zoom out')} onClick={() => zoomCamera('out')} disabled={status !== 'ready'} className="grid size-11 place-items-center rounded-lg text-2xl font-light text-muted transition-[color,background-color,transform] hover:bg-surface-raised hover:text-ink active:scale-95 disabled:cursor-wait disabled:opacity-40 motion-reduce:active:scale-100">−</button>
          <span className="mx-2 h-px bg-line" aria-hidden="true" />
          <button type="button" aria-label={t(locale, 'Reset 3D body view')} onClick={resetCamera} disabled={status !== 'ready'} className="grid size-11 place-items-center rounded-lg text-muted transition-[color,background-color,transform] hover:bg-surface-raised hover:text-ink active:scale-95 disabled:cursor-wait disabled:opacity-40 motion-reduce:active:scale-100">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-3">
        <div className="flex min-w-0 flex-1 items-center gap-1 overflow-x-auto rounded-xl border border-line bg-canvas p-1 sm:flex-none" role="group" aria-label={t(locale, 'View body from')} aria-controls={viewerId}>
          {(['front', 'back', 'left', 'right'] as const).map((view) => (
            <button key={view} type="button" aria-label={t(locale, `${view[0].toUpperCase()}${view.slice(1)}`)} aria-pressed={activeView === view} onClick={() => chooseView(view)} disabled={status !== 'ready'} className={activeView === view ? 'min-h-10 flex-1 rounded-lg bg-brand/14 px-3 text-sm font-semibold capitalize text-brand disabled:cursor-wait disabled:opacity-40 sm:flex-none' : 'min-h-10 flex-1 rounded-lg px-3 text-sm font-semibold capitalize text-muted transition-[color,background-color] hover:bg-surface-raised hover:text-ink disabled:cursor-wait disabled:opacity-40 sm:flex-none'}>{t(locale, view)}</button>
          ))}
        </div>
        <p className="text-xs leading-5 text-subtle sm:ml-auto">{t(locale, 'Pinch or scroll to zoom')} <span aria-hidden="true">·</span> {t(locale, 'Two-finger or right-drag to pan')}</p>
      </div>
      <p className="mt-3 text-xs leading-5 text-subtle">{t(locale, '3D body model:')} <a className="text-muted underline decoration-line-strong underline-offset-4 hover:text-ink" href="https://humanatlas.io/3d-reference-library" rel="license">HuBMAP Human Reference Atlas</a>, {t(locale, 'licensed under')} <a className="text-muted underline decoration-line-strong underline-offset-4 hover:text-ink" href="https://creativecommons.org/licenses/by/4.0/" rel="license">CC BY 4.0</a>.</p>
    </div>
  );
}
